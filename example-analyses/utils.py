...
def read_responses(contest, duplicates=None):
    ...
    filename = "{dir_}/../contests/responses/{contest}".format(
    ...
    with ZipFile("{filename}".format(filename=filename)) as myzip:
        ...
        df["timestamp_query_generated"] = pd.to_datetime(
            df["timestamp_query_generated"]
        )
    ...
    if duplicates == "ignore" or any(np.isnan(df["target_id"].unique())):
        captions = {caption: k for k, caption in enumerate(df["target"].unique())}
...
def add_target_ids(summary, responses):
    mapping = {
        target: k for target, k in zip(responses["target"], responses["target_id"])
    }
...
def add_captions(summary, responses):
    mapping = {
        k: target for target, k in zip(responses["target"], responses["target_id"])
    }
...
def add_response(
    summary, response, mapping={1.0: "unfunny", 2.0: "somewhat_funny", 3.0: "funny"}
):
...
def summary_from_responses(
    responses, alg=None, mapping={1: "unfunny", 2: "somewhat_funny", 3: "funny"}
):
    if alg:
...
def read_all_responses():
    ...
    for filename in os.listdir(dir_ + "/../contests/responses/"):
        if "Store" in filename or any(
            [d in filename for d in ["497", "499", "dueling"]]
        ):
            continue
...
def read_all_summaries():
    ...
    """
    dfs = {
        filename: read_summary(filename)
        for filename in os.listdir(dir_ + "/../contests/summaries")
        if not any(
            [d in filename for d in ["497", "499", "dueling", "munging", "Store"]]
        )
        and filename[0] != "_"
    }
